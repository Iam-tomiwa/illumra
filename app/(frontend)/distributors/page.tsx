...
import StoreLocator, {
  StoreInput,
} from "@/components/store-locator/store-locator";
import { sanityFetch } from "@/sanity/lib/fetch";
import {
  STORES_QUERY,
  distributorsPageQuery,
  settingsQuery,
...
export async function generateMetadata(): Promise<Metadata> {
  const [distributorsPage, settings] = await Promise.all([
    sanityFetch({ query: distributorsPageQuery }),
    sanityFetch({ query: settingsQuery, revalidate: 3600 }),
  ]);

  const mergedSeo = mergeSeo(distributorsPage?.seo, settings?.seo);
...
export default async function DistributorsPage() {
  ...
  const backgroundImage = resolveBackgroundUrl(
    distributorsPage?.backgroundImage as Parameters<
        ...
        <div className="max-w-3xl">
          <AnimatedElement delay={0.1}>
            <h1 className="page-title">
              {distributorsPage?.pageTitle || "Where to Buy"}
            </h1>
          </AnimatedElement>
          ...
          {!distributorsPage?.description && (
            <AnimatedElement delay={0.2}>
              <p className="mb-6">
                Discover local distributors with ease! Simply search by city,
                state, or ZIP code, or use the &quot;Near Me&quot; button to
                find the closest distributors to your location. We&apos;re here
                to help you connect with the best options available.
              </p>
            </AnimatedElement>
          )}
        </div>
