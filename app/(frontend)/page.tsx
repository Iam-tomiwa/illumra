...
export default async function WirelessControlSolutionsLandingPage() {
  ...
  const featuredProductsVisible =
    sanitizeSanityStrings(homePage?.featuredProductsVisible) !== "hide";
  const faqVisible = sanitizeSanityStrings(faq?.visible) !== "hide";
  const testimonialsVisible =
    sanitizeSanityStrings(homePage?.testimonialsVisible) !== "hide";
  const projectsVisible =
    sanitizeSanityStrings(homePage?.projectsVisible) !== "hide";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {sanitizeSanityStrings(controlSolutions?.visible) === "show" &&
        controlSolutions?.features &&
        controlSolutions.features.length > 0 && (
          <EnergyFeatures section={controlSolutions} />
        )}
      {sanitizeSanityStrings(trustedBy?.visible) === "show" &&
        trustedBy?.logos &&
        trustedBy.logos.length > 0 && <TrustedBy section={trustedBy} />}
      {sanitizeSanityStrings(about?.visible) === "show" && (
      ...
      {projectsVisible && projects && projects.length > 0 && (
        <ProjectsSection
          ...
          projectsHeading={homePage?.projectsHeading}
          projectsSubheading={homePage?.projectsSubheading}
        />
      )}
