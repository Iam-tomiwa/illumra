...
export function seoToMetadata(
  ...
  // Sanitize SEO data to remove special characters from strings
  const sanitizedSeo = sanitizeSanityData(seo) as typeof seo;

  const metadataBase = sanitizedSeo.siteUrl
    ? new URL(sanitizedSeo.siteUrl)
    : undefined;
  const ogImage = resolveMediaAsset(sanitizedSeo.ogImage as any, {
    width: 1200,
    height: 627,
  });

  const keywordsString = sanitizedSeo.keywords
  ...
  const metadata: import("next").Metadata = {
    ...
    ...(sanitizedSeo.title && {
      title: {
        template: sanitizedSeo.titleTemplate || `%s | ${sanitizedSeo.title}`,
        default: sanitizedSeo.title,
      },
    }),
    ...(sanitizedSeo.description && { description: sanitizedSeo.description }),
    ...(keywordsString && { keywords: keywordsString }),
    openGraph: {
      type: "website",
      ...(sanitizedSeo.locale && { locale: sanitizedSeo.locale }),
      ...(metadataBase && { url: metadataBase }),
      ...(sanitizedSeo.title && {
        siteName: sanitizedSeo.title,
        title: sanitizedSeo.title,
      }),
      ...(sanitizedSeo.description && {
    ...
    twitter: {
      ...(sanitizedSeo.twitterCard && {
        card: sanitizedSeo.twitterCard as
          | "summary"
          | "summary_large_image"
          | "app"
          | "player",
      }),
      ...(sanitizedSeo.title && { title: sanitizedSeo.title }),
      ...(sanitizedSeo.description && {
        description: sanitizedSeo.description,
      }),
      ...(ogImage?.url && { images: [ogImage.url] }),
      ...(sanitizedSeo.twitterSite && { site: sanitizedSeo.twitterSite }),
      ...(sanitizedSeo.twitterCreator && {
        creator: sanitizedSeo.twitterCreator,
      }),
    },
  };
