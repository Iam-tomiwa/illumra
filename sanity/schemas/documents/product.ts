...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      group: "content",
      description:
        "Mark this product as a top seller to display the Top Selling badge.",
      options: {
        list: [
          { title: "Yes", value: "yes" },
          { title: "No", value: "no" },
    ...
    defineField({
      ...
      group: "content",
      rows: 3,
      description: "Short marketing copy shown on summaries for SEO.",
      validation: (rule) => [
        rule
          .min(40)
          .max(320)
    ...
    defineField({
      ...
      description:
        "Select related products to display on the product detail page.",
      of: [
        defineArrayMember({
          type: "reference",
          to: [{ type: "product" }],
          options: {
            filter: `_type == "product" && !(_id in path("drafts.**")) && _id != $id`,
            filterParams: { id: "_id" },
          },
        }),
      ],
      validation: (rule) => [
        rule.unique().warning("Duplicate related products were removed."),
      ],
    }),
    defineField({
      name: "specifications",
    ...
    defineField({
      ...
      group: "media",
      description:
        "Gallery images displayed in product cards and detail pages.",
      of: [
        defineArrayMember({
          type: "mediaAsset",
        }),
        ...
        defineArrayMember({
          ...
          fields: [
            defineField({
              ...
              type: "string",
              validation: (rule) => [
                rule.required().error("Add an application title."),
              ],
            }),
            defineField({
              name: "url",
    ...
    defineField({
      ...
      group: "seo",
      description:
        "Override default SEO settings for this product page (optional)",
    }),
  ],
  preview: {
    select: {
