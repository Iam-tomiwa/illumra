...
export function ProductImageSlider({
  ...
  const validImages = images.filter(
    (img): img is ResolvedMediaAsset =>
      img !== undefined && img.url !== undefined
  );

  const validVideos = (videos ?? []).filter(
    (vid): vid is VideoItem =>
      vid !== undefined && vid.externalUrl !== undefined
  );
        ...
        <div className="relative">
          ...
          >
            ...
            {validVideos.map((vid, index) => {
              ...
              return (
                <SwiperSlide
                  key={`video-thumb-${vid._key || index}`}
                  className="size-20! cursor-pointer"
                >
                  <div
                    className={`relative w-full h-full rounded-lg overflow-hidden border-2 transition-all ${
                      activeIndex === validImages.length + index
                        ? "border-primary"
                        : "border-transparent hover:border-muted-foreground/50"
                    }`}
                    onClick={() => {
