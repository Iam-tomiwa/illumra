...
const Navbar: React.FC<{
	...
	settings,
		...
		// Resolve logo from settings or use default
		const logoAsset = resolveMediaAsset((settings?.logo ?? undefined) as Parameters<typeof resolveMediaAsset>[0]);
		const logoUrl = logoAsset?.url || "/illumra-logo.png";
		const logoAlt = logoAsset?.alt || "Logo";

		// Get CTA button config from settings or use defaults
		const ctaLabel = settings?.ctaButton?.label || "Get A Quote";
		const ctaHref = settings?.ctaButton?.href || "/contact";
		const ctaIcon = settings?.ctaButton?.icon || "solar:round-arrow-right-bold";
		...
		return (
			<nav className={cn("sticky w-full top-0 z-50 bg-background", className)}>
					...
					<div className="flex justify-between items-center h-16">
						...
						<Link href="/" className="flex items-center lg:ml-4 h-10 relative">
							{logoUrl.startsWith("http") ? (
								...
								// Internal path - use Next.js Image
								<Image
									src={logoUrl}
									alt={logoAlt}
									height={40}
								...
								link.submenu ? (
									...
									>
										...
										>
											{link.name}
											<ChevronDown
												className={`size-6 transition-transform duration-200 ${isSubMenuOpen === link.name ? "rotate-180" : ""
													}`}
											/>
										</button>
										{/* Submenu */}
										<div
											className={`absolute min-w-[200px] overflow-clip left-0 mt-2 bg-background border rounded-xl shadow-lg text-black w-max transition-all duration-200 ${isSubMenuOpen === link.name
						...
						<div className="hidden lg:flex items-center">
							<Button asChild className="px-8! shadow-lg shadow-primary/20">
								<Link href={ctaHref}>
									{ctaLabel}
									{ctaIcon && <Icon icon={ctaIcon} className="size-7" />}
								</Link>
							</Button>
						</div>
				...
				{/* Mobile Menu */}
				<div
					className={`fixed top-[64px] bottom-0 left-0 w-full bg-background transition-[clip-path] duration-500 ease-in-out overflow-y-auto z-40 ${!isMobileMenuOpen
				...
				>
					<div className="py-12 space-y-6 w-[90%] mx-auto">
							...
							link.submenu ? (
								<div key={link.name} className="space-y-2">
									...
									>
										{link.name}
										<ChevronDown
											className={`w-5 h-5 transition-transform duration-200 ${isSubMenuOpen === link.name ? "rotate-180" : ""
												}`}
										/>
									</button>
						...
						<Button asChild className="px-8! shadow-lg shadow-primary/20">
							<Link onClick={toggleMobileMenu} href={ctaHref}>
								{ctaLabel}
								{ctaIcon && <Icon icon={ctaIcon} className="size-7" />}
							</Link>
						</Button>
